...
function generateNo($koneksi) {
    $query = "SELECT MAX(no) as max_no FROM poltek_jember";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($action == 'add') {
        $query = mysqli_query($koneksi, "INSERT INTO poltek_jember (no, no_bmn, nama_barang, spesifikasi, tahun_pengadaan, kondisi_baik, rusak_ringan, rusak_berat, jumlah_barang, durasi_pemakaian, fungsi_alat, stock_barang, foto) VALUES ('$no', '$no_bmn', '$nama_barang', '$spesifikasi', '$tahun_pengadaan', '$kondisi_baik', '$rusak_ringan', '$rusak_berat', '$jumlah_barang', '$durasi_pemakaian', '$fungsi_alat', '$stock_barang', '$nama_gambar_baru')");
        if ($query) {
...
if ($action == 'edit' && $id != '') {
    $result = mysqli_query($koneksi, "SELECT * FROM poltek_jember WHERE no='$id'");
    $data = mysqli_fetch_assoc($result);
}
?>
    ...
    <div class="mx-auto">
        <div class="card">
            <div class="card-header">
                <?php echo $action == 'edit' ? 'Edit Data' : 'Add / Tambah Data'; ?>
            </div>
            <div class="card-body">
                <form action="" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-sm-10">
                            ...
                            <?php if ($action == 'edit' && isset($data['foto'])) : ?>
